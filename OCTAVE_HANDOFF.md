...
A long session. Three things matter more than the rest, and two of them are
corrections to claims made earlier the same day.

### 1. Use `raw_case_fraction`, never the reward, to say "the policy improved"

**This invalidated a headline finding before it was caught.** `case_fraction`
is the *discounted* reward: a fully correct answer earns 1.00 on attempt 1,
**0.85 on attempt 2, 0.60 on attempt 3**. Thresholding it at 0.999 to define
"solved" therefore cannot count any success after the first attempt.

Doing exactly that produced the conclusion "retries raise reward but not solve
rate", which is false. Recomputed correctly:
...
unaffected and why this survived.

### 2. Almost none of that comes from the feedback text

A control replaced the entire retry diagnostic with the sentence "That answer
was not correct." Nemotron, 2 turns, guide off, same tasks and seed:

| level | 1 turn | content-free retry | full diagnostic | difference |
|---|---:|---:|---:|---:|
| L1 | 0.570 | 0.688 | 0.719 | +0.031 (t 0.98) |
| L2 | 0.504 | 0.656 | 0.672 | +0.016 (t 0.49) |
| L3 | 0.309 | 0.434 | 0.441 | +0.008 (t 0.21) |

**A retry that says nothing captures 79-95% of the gain.** Observed two-turn
solve also sits *below* independent resampling (0.688 against the 0.815 that
`1-(1-p)^2` predicts), so the scaffold is approximately **correlated best-of-N
sampling** with a small informational bonus.

The one component that does pay is the **LLM guide** — a specific,
model-generated diagnosis of the actual bug — worth **+0.062 at L1 (t = 2.37)**
on top of the extra attempt, where generic text is worth nothing.

**Consequence for reporting:** a multi-turn score is mostly resampling, not
capability. Always state the turn budget.

### 3. The feedback was rebuilt anyway, and it was right to

Measured over 396 retries, the old message was raw Octave stdout: **46% of its
length was the `__OCTAVE_CANDIDATE_RESULT__` transport blob**, identical errors
repeated once per case each with a random temp path, and **33% of retries
carried no diagnostic at all** — the model was told "0/6" and shown its own
output values.

`build_retry_feedback` now strips the blob and paths, deduplicates errors, and
names which of three failure modes occurred (did not run / wrong shape, giving
both shapes / right shape, wrong values). A six-case syntax failure went from
1,084 characters to 110. The guide fires **on need** — the first retry whose
diagnostic cannot help — rather than at a fixed attempt, and the reward discount
now follows the hint rather than the attempt number.
...
- **The guide is worth keeping on**, but it is the only part of the feedback
  apparatus with a measured effect.
