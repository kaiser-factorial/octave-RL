...
def test_solved_is_reported_undiscounted() -> None:
    ...
    # it has to mean the same thing whatever the turn budget, and whatever
    # `reward_mode` is set to -- which is why it is the field to read the
    # ablation arms through.
...
def test_solved_only_pays_nothing_for_code_that_merely_runs() -> None:
    # Partial case credit is the only channel through which an answer that is
    # not fully correct can be worth anything -- there has been no execution or
    # structured-output bonus since the 2026-08-05 hardening -- so closing it
    # makes the reward strictly "right answer or nothing". Measured on the
    # 2026-07-29 distribution, that channel is 5 of 200 rollouts and about 6% of
    # total reward mass.
